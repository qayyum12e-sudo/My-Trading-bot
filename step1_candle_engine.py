"""
MAIN — Milele Prime Bot Entry Point  (Bybit Futures Edition)
=============================================================
"""
...
print("BYBIT_API_KEY loaded:", bool(os.environ.get("BYBIT_API_KEY")))
print("BYBIT_API_SECRET loaded:", bool(os.environ.get("BYBIT_API_SECRET")))
...
def _bybit_sign(params_str: str):
    ts  = str(int(time.time() * 1000))
    raw = ts + API_KEY + RECV_WINDOW + params_str
...
@app.route('/proxy/bybit/wallet')
def proxy_bybit_wallet():
    return jsonify(bybit_signed_get('/v5/account/wallet-balance',
                                    {'accountType': 'UNIFIED'}))


@app.route('/proxy/bybit/positions')
def proxy_bybit_positions():
    sym    = request.args.get('symbol', '')
    params = {'category': 'linear'}
    if sym:
        params['symbol'] = sym
    return jsonify(bybit_signed_get('/v5/position/list', params))


@app.route('/proxy/bybit/orders')
def proxy_bybit_orders():
    sym    = request.args.get('symbol', '')
    params = {'category': 'linear'}
    if sym:
        params['symbol'] = sym
    return jsonify(bybit_signed_get('/v5/order/realtime', params))


@app.route('/proxy/bybit/ticker')
def proxy_bybit_ticker():
    sym = request.args.get('symbol', '')
    url = f'{BYBIT_BASE}/v5/market/tickers?category=linear&symbol={sym}'
...
def proxy_stats():
    ...
    return jsonify({'consec_losses': {'S1': 0}, 'open_count': 0})
...
def main():
    global detector, engine, manager

    print(f"""
+------------------------------------------------------+
|  MILELE PRIME  --  Bybit Futures Edition             |
|  Strategy : S1 EMA 9/26 Cross  $20 x up to 50x      |
|  Symbols  : {len(SYMBOLS)} coins                              |
|  Timeframe: 15m candles                              |
|  Mode     : {'DEMO (paper money)' if TESTNET else 'LIVE'}                       |
...
""")
    ...
    log.info("Milele Prime ready. Starting candle engine...")
...
def _start_bot_once():
    ...
    with _bot_thread_lock:
        if _bot_thread is not None and _bot_thread.is_alive():
            return
        log.info("Starting Milele Prime trading thread...")
