...
# Strategy 1
S1_ADX_MIN        = 25.0
S1_SL_PCT         = 0.5    # minimum SL floor (used if ATR-based SL is smaller)
S1_TP_PCT         = 1.5    # used only as fallback if ATR is unavailable

# ATR-based SL/TP: SL = max(S1_SL_PCT, ATR_MULT * atr_pct), TP = SL * RR
# This adapts to current market volatility instead of using a flat 0.5% that gets
# chopped out by normal noise. Backtest used flat % which assumed zero slippage —
# ATR-based stops give breathing room on volatile candles.
S1_ATR_SL_MULT    = 1.5    # SL = 1.5 × ATR%  (or 0.5% floor, whichever is greater)
S1_RR_RATIO       = 3.0    # TP distance = 3 × SL distance (keeps your 1:3 backtest ratio)
...
class SignalDetector:
    ...
    def _check_s1(self, symbol: str, candle: dict, ind: dict, state: SymbolState):
        ...
        # ── ATR-based SL/TP (Fix #3 — volatility-adaptive stops) ─────────────
        # If ATR% is available, use max(floor, ATR_MULT × ATR%) for SL distance.
        # Otherwise fall back to flat percentages (legacy behaviour).
        atr_pct = ind.get('atr_pct')
        if atr_pct is not None and atr_pct > 0:
            sl_dist_pct = max(S1_SL_PCT, S1_ATR_SL_MULT * atr_pct)
            tp_dist_pct = sl_dist_pct * S1_RR_RATIO
        else:
